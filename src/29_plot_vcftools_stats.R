...
im <- read.table("missing.imiss", sep="\t", stringsAsFactors = F, header = T)
hist(im$F_MISS)
im %>%
  filter(F_MISS < 0.8) %>%
  count()
  
df <- read.table("maf.frq", row.names=NULL, sep="\t", stringsAsFactors = F, header=T, fill=NA)
  if(nrow(df) > 0){
    ...
    miss_df <- read.table("missing.lmiss", sep="\t", stringsAsFactors = F, header = T)
    miss_df
    df2[,"F_MISS"] <- miss_df[,6]
    
    hwe <- read.table("hwe.hwe", sep="\t", stringsAsFactors = F, header = T)
