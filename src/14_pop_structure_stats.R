...
# system(paste(plink,"--bfile",ldfile,"--allow-extra-chr","--indep-pairwise 250 10 0.3","--out",ldfile))
...
dim(vcf4)
vcf4 <- set.stats(vcf4)
plot(density(vcf4@snps$callrate))
...
vcf5 <- select.snps(vcf4, callrate > 0.5 & maf > 0.1)
vcf5 <- set.stats(vcf5)
...
dim(vcf6)
write.csv(vcf6@ped, "working_set_1660_samples.csv")
write.csv(vcf6@snps, "working_set_5362_snps.csv")

barplot(table(vcf5@snps$chr))
dim(vcf5)
...
hierfstat_vcf <- hierfstat::read.VCF("merged.vcf.gz", )
...
# boot.vc(hfs[,1],hfs[,-1])
# 
fs.gt <- fs.dosage(X,pop=pop)
fs.pgt <- pairwise.fst.dosage(X,pop=pop)

image(1:4,1:4,fs.gt$FsM,main=expression(F[ST]^{XY}))
