rm(list=ls())
setwd("/fs/scratch/PAS2444/jignacio/2024/pm/data/d21_gc_content_flanks")
...
plot(density(df$Right.GC))
abline(v=80, lty=2)
abline(v=34, lty=2)
df2 <- df %>%
  filter(Left.GC >= 35 & Left.GC <= 65,
         Right.GC >= 35 & Right.GC <= 65)

snps <- read.csv("/fs/scratch/PAS2444/jignacio/2024/pm/data/d17_filtered_high_call_rate_vcf_stats/SNP_repeats_removed.csv")
